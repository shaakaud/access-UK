...
function! s:GitDiff(...)
    ...
    let prefix = system("/ws/lakskuma-bgl/software/git/git-1.7.10/postinstall/bin/git rev-parse --show-prefix")
    let thisfile = substitute(expand("%"),getcwd(),'','')
    let gitfile = substitute(prefix,'\n$','','') . thisfile

    " Check out the revision to a temp file
    let tmpfile = tempname()
    let cmd = "/ws/lakskuma-bgl/software/git/git-1.7.10/postinstall/bin/git show  " . rev . ":" . gitfile . " > " . tmpfile
...
function! s:GitChanges(...)
  ...
  let gitfile = substitute(prefix,'\n$','','') . thisfile
	...
	let diff = system("diff -u0 <(/ws/lakskuma-bgl/software/git/git-1.7.10/postinstall/bin/git show " . rev . ":" . gitfile . ") <(cat;echo)", contents)
