...
#
#   $Id: .bashrc,v 1.3 2010/06/08 14:31:53 smorton Exp $
#
###########################################################################
#
...
# Set the DISPLAY - UDAY
#export DISPLAY=10.135.20.161:1.0
#export DISPLAY=135.254.172.188:0.0
...
#
# Do some poking around to see if the environment is OK
#
if $(uname -a | grep -qi cygwin)
then
    if [ ! -d $WIND_BASE ]
    then
        echo "You do not appear to have Tornado installed in $WIND_BASE"
    elif [ ! -d $WIND_BASE/host/${WIND_HOST_TYPE}/bin ]
    then
        echo "You do not appear to have the right cross compiler tools"
    fi
fi
...
alias cvschanged='cvs -z9 -q status | grep Status | egrep -v "(Up-to|Needs Patch)"'
alias cvschanged2="cvs -nqr update -dPR | grep '^[MARPC] '"
alias cvsstatus='cvs status | grep Status'
alias cvschanged="cvs -n -q up -dPR 2>/dev/null | grep '^[MAR?] '"
